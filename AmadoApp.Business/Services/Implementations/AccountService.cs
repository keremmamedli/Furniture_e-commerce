...
namespace AmadoApp.Business.Services.Implementations
{
	public class AccountService : IAccountService
	{
		...
		public async Task<AppUser> GetUserByEmailAddressAsync(string email)
		{
			return await _userManager.FindByEmailAsync(email);
		}

		public async Task<bool> ChangePasswordAsync(ChangePasswordVM vm, string email)
		{
			var currentUser = await _userManager.FindByEmailAsync(email);

			var token = await _userManager.GeneratePasswordResetTokenAsync(currentUser);
			var result = await _userManager.ResetPasswordAsync(currentUser, token, vm.NewPassword);
			...
			else
			{
				...
				throw new ChangePasswordException($"Change password failed!{errorMessages}", nameof(vm.NewPassword));
			}
		}
